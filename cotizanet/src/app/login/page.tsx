...
import { getCurrentUser, loginAdmin, logoutAdmin } from "@/app/lib/api";
import { setAdminSessionCookie } from "@/app/lib/session";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const roles = extractNames(profile.roles);
      const groups = extractNames(profile.groups);
      const hasAdminAccess = roles.includes("ROLE_ADMIN");
      const hasSalesAccess = roles.includes("ROLE_SALES") || roles.some((role) => role.toLowerCase() === "ventas");
      const hasSalesGroup = groups.some((group) => group.toLowerCase() === "ventas");

      if (!hasAdminAccess && !hasSalesAccess && !hasSalesGroup) {
        await logoutAdmin().catch(() => undefined);
        setError("Tu cuenta no tiene acceso. Usa una cuenta con rol ROLE_ADMIN o ROLE_SALES, o grupo ventas.");
        return;
      }
      ...
      setError(
        message.includes("401") || message.includes("403")
          ? "Credenciales inválidas o cuenta sin acceso."
          : message,
      );
    } finally {
      setLoading(false);
    ...
    <main className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,rgba(24,24,27,0.08),transparent_38%),linear-gradient(180deg,#fafafa_0%,#f4f4f5_100%)] px-4">
      ...
      >
        ...
        <div className="space-y-2 text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-zinc-500">Cotizanet</p>
          <h1 className="text-2xl font-semibold text-zinc-950">Iniciar sesión</h1>
          <p className="text-sm text-zinc-500">Cotizanet es una plataforma de cotizaciones para clientes.</p>
        </div>
